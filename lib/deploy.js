...
const { successLog, errorLog, tipsLog } = require('../utils/index');
...
async function deploy(config) {
  ...
  try {
    ...
    successLog(`\n${tipsLog(name)}: ${tipsLog(projectName)} 项目部署成功\n`);
    process.exit(0);
  } catch (err) {
    errorLog(`  部署失败 ${err}`);
    process.exit(1);
...
async function uploadFile(webDir) {
  try {
    console.log(`===== 上传zip至目录${tipsLog(webDir)} =====`);
...
async function unzipFile(webDir) {
  try {
    ...
    await runCommand(`rm -f ./umi.*`, webDir);
